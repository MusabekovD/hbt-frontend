...
const BathroomDetails = () => {
  const [bathDetail, setBathDetail] = useState(null);
  const { id } = useParams();
const aboutUrl = bathDetail?.attributes.photo_about.data[0]?.attributes.formats.large.url
  useEffect(() => {
    fetch(`https://hbt-backend.onrender.com/api/bathrooms/${id}?populate=*`)
      .then((res) => res.json())
      .then((data) => setBathDetail(data.data));
  }, [id]);

  console.log(bathDetail);
  return (
      ...
      <div className="flex lg:flex-row flex-col lg:gap-8 sm:gap-10 gap-12">
        <div className="w-full lg:w-6/12">
          <h2 className="w-full font-bold lg:text-4xl text-3xl lg:leading-10 leading-9">
            {bathDetail?.attributes.name}
          </h2>
          <p className="font-normal text-base leading-6 text-gray-600 mt-6">
          {bathDetail?.attributes.description}
          </p>
        </div>
        <div className="w-full lg:w-6/12">
          <img
